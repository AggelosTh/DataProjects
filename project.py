...
Original file is located at
    https://colab.research.google.com/drive/1FHDHAJ_WAF-D3bi-AehTnMAn4BpZ7BWp
"""
...
# Drop rows where humidity out is NaN in dff
dff = dff.dropna(subset=['humidity_out'])
dff.humidity_out.isnull().sum()

dff

# create a new column in order to fill it with the new values of temp_out
from numpy import nan
...
# Train model in order to compute temperature in from humidity out, temp out and ac_power
X = a.iloc[:, [0,3,4]]
y = a.iloc[:, 2]
...
# Train model in order to copmpute humidity in from του humidity out and temperature in
...
# At this point we can develop the model so that it can decides to keep the power on, if there is motion in 15 minutes around the specific time
